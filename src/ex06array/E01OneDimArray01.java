...
/*
 ...
 -생성된 배열의 힙(Heap)영역에 저장되고, 할당된 메모리의 주소값을 반환한다.
...
public class E01OneDimArray01 {
	...
	public static void main(String[] args) {
		...
		System.out.println("numArr의의 0번방의 값:"+numArr[0]);
		System.out.println("numArr의의 1번방의 값:"+numArr[1]);
		System.out.println("numArr의의 2번방의 값:"+numArr[2]);
		//정수형 배열이 Heap영역에 생성된 후 반환된 주소값이 출력된다.
		System.out.println("배열명(numArr)="+numArr);
		System.out.println("=========================");
		
		//배열의 선언 및 초기화 : 배열은 여러개의 변수를 사용하기 위해 선언하므로 접근을 위해 일반적으로 반복문(for)을 사용한다.
		//배열명.length : 해당 배열의 크기를 반환한다. 배열의 크기가 변경되는 경우 유연하게 대처할 수 있다.
		int[] arrNumber = new int[40];
		//배열의 각 원소를 10부터 1씩 증가하면서 초기화 한다.
		...
		//가장 기본적인 배열 선업과 초기화방법
		System.out.println("방법1 : 배열의 선언과 초기화");
		//크기가 1인 배열을 생성한 후 인덱스로 접근하여 초기화한다.
		int[] initArr = new int[1];
		initArr[0] = 100;
		System.out.println("initArr[0]="+ initArr[0]);
		
		/* 배열을 선업과 동시에 초기화하는 경우 자바 컴파일러는 초기값의 갯수를 통해 배열의 크기를 결정할 수 있다.
		 따라 이 경우에는 크기는 생략하도록 약속되어 있다.
		 */
