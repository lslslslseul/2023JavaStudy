...
//-Stack은 입구가 하나만 있는 컵과 같은 형태이며, '선입후출'(먼저 입력한 선언이 맨 마지막에 소멸됨)의 특성을 가진다. 
...
//		main지역에서 num을 선언하면 그보다 좁은 지역인 if문에서는 같은 이름의 변수를 선언할 수 없다. 큰 지역에서 이미 선언되어 메모리에 상주하고 있기 때문이다.
//		int num = 9;
...
//	main에서 선언한 변수 num은 메모리에 아직 상주하고 있지만 해당 메서드는 크기가 동일한 서로 다른 지역에 정의되었으므로 같은 이름의 변수를 선언할 수 있다.
//	앞인 main메서드의 if~else문의 블럭도 같은 개념으로 변수를 선언한 것이다.
